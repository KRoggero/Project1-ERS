...
public class HomeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (Reimbursement reimbursement : reimburseList) {
			...
			if (flag == 1) {
				out.println("<td> <a href='AlterStatusServlet?approve=yes&id=" +reimbursement.getId()
				+"'> Approve </a> ||");
				out.println("<a href='AlterStatusServlet?approve=no&id=" +reimbursement.getId()
				+"'> Deny </a> </td>");
			}
			out.println("</tr>");
		}
		...
		if (role.equals(Role.EMPLOYEE)) {
		out.println("<a href='createreimbursement.html'> Submit Reimbursement Request");
		}
	}
